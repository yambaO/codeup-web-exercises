...
function setWeatherData() {
    ...
    fetch(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${SALat}&lon=${SALong}&appid=${OPEN_WEATHER_APPID}&units=imperial`
    )
...
submitButton.addEventListener("click", () => {
    geocode(searchedCity.value, MAPBOX_API_TOKEN)
        .then((data) => {
            map.setCenter(data);
            setWeatherData();
...
reverseGeocode({}, MAPBOX_API_TOKEN).then((data) => console.log(data));
